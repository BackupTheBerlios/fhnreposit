...
if ( empty ( $id ) ) {
  ...
  $res = dbi_query ( "SELECT cat_id, cat_owner, cat_picto FROM webcal_categories " .
    "WHERE cat_id = $id" );
  if ( $res ) {
...
if ( empty ( $error ) &&
  ...
  if ( ! empty ( $id ) ) {
    ...
    $sql = "UPDATE webcal_categories SET cat_name='$catname', cat_picto='$catpicto' " .
      "WHERE cat_id = $id";
    if ( ! dbi_query ( $sql ) ) {
      $error = translate ("Database error") . ": " . dbi_error();
    }
    ...
    if ( $res ) {
      ...
      $sql = "INSERT INTO webcal_categories " .
        "( cat_id, cat_owner, cat_name, cat_picto ) " .
        "VALUES ( $id, $catowner, '$catname', '$catpicto' )";
      if ( ! dbi_query ( $sql ) ) {
        $error = translate ("Database error") . ": " . dbi_error();
      }
    } else {
