...
<a title="<?php etranslate("Return Admin") ?>" class="nav" href="adminhome.php">&laquo;&nbsp;<?php etranslate("Return Admin") ?></a><br /><br />
...
<tr><td class="tooltip" title="<?php etooltip("timed-evt-len-help");?>"><?php etranslate("Specify timed event length by")?>:</td>
  <td><label><input type="radio" name="pref_TIMED_EVT_LEN" value="D" <?php if ( $prefarray["TIMED_EVT_LEN"] != "E" ) echo " checked=\"checked\"";?> /> <?php etranslate("Duration")?></label> <label><input type="radio" name="pref_TIMED_EVT_LEN" value="E" <?php if ( $prefarray["TIMED_EVT_LEN"] == "E" ) echo " checked=\"checked\"";?> /> <?php etranslate("End Time")?>
</label></td></tr>
<?php } ?>
...
<tr><td style="font-weight:bold;"><label for="pref_cat"><?php etranslate("Default Category")?>:</label></td>
  <td><select name="pref_CATEGORY_VIEW" id="pref_cat" disabled>
  <?php
  echo "<option value=\"\"";
...
<?php if ( $PUBLISH_ENABLED == 'Y' && $is_admin) { ?>
<table class="standard" cellspacing="1" cellpadding="2">
