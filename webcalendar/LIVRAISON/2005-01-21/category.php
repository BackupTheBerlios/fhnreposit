...
if ( ! empty ( $id ) ) {
  $res = dbi_query ( "SELECT cat_id, cat_owner, cat_picto FROM webcal_categories WHERE " .
    "cat_id = $id" );
  if ( $res ) {
...
if ( ( ( $add == '1' ) || ( ! empty ( $id ) ) ) && empty ( $error ) ) {
  ...
  if ( ! empty ( $id ) ) {
    ...
    $catpicto = $category_picto[$id];
  }
  ?>
  <?php etranslate("Category Name")?> : <br />
  <input type="text" name="catname" size="20" value="<?php echo htmlspecialchars ( $catname ); ?>" /><br />
  <br />
  <?php etranslate("Category Picto")?> :<br />
  <input type="text" name="catpicto" size="40" value="<?php echo htmlspecialchars ( $catpicto ); ?>" />
  <br />
