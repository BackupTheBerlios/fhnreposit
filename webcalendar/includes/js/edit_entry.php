...
function validate_and_submit () {
  ...
  if ( document.forms[1].hour.value.charAt ( 0 ) == '0' )
    document.forms[1].hour.value = document.forms[1].hour.value.substring ( 1, 2 );
  if ( document.forms[1].timetype.selectedIndex == 1 ) {
    h = parseInt ( document.forms[1].hour.value );
    m = parseInt ( document.forms[1].minute.value );
<?php if ($GLOBALS["TIME_FORMAT"] == "12") { ?>
    if ( document.forms[1].ampm[1].checked ) {
      // pm
      if ( h < 12 )
        h += 12;
    } else {
...
<?php } ?>
    if ( h >= 24 || m > 59 ) {
      ...
      document.forms[1].hour.select ();
      document.forms[1].hour.focus ();
      return false;
    }
    ...
    <?php if ($GLOBALS["TIME_FORMAT"] == "24") {
      ...
      echo "if ( h < $WORK_DAY_START_HOUR && document.forms[1].ampm[0].checked ) {";
    }
    ?>
...
?>function selectUsers () {
  ...
  for ( i = 0; i < document.forms[1].elements.length; i++ ) {
    if ( document.forms[1].elements[i].name == "participants[]" )
      listid = i;
  }
  url = "usersel.php?form=editentryform&listid=" + listid + "&users=";
  // add currently selected users
  for ( i = 0, j = 0; i < document.forms[1].elements[listid].length; i++ ) {
    if ( document.forms[1].elements[listid].options[i].selected ) {
      if ( j != 0 )
  url += ",";
      j++;
      url += document.forms[1].elements[listid].options[i].value;
    }
  }
  //alert ( "URL: " + url );
  // open window
...
?>function timetype_handler () {
  var i = document.forms[1].timetype.selectedIndex;
  var val = document.forms[1].timetype.options[i].text;
  //alert ( "val " + i + " = " + val );
  // i == 1 when set to timed event
  if ( i != 1 ) {
    // Untimed/All Day
    makeInvisible ( "timeentrystart" );
    if ( document.forms[1].duration_h ) {
...
function rpttype_handler () {
  var i = document.forms[1].rpttype.selectedIndex;
  var val = document.forms[1].rpttype.options[i].text;
  //alert ( "val " + i + " = " + val );
