...
// fichier d'orgine user.php;
// modifié pour auhthentification à l'aide du service web HN
include_once('ldapsweb_fct.php');
...
function user_valid_login ( $login, $password ) {
  ...
  $parametres[passwd]=$password; // l'encryption est faite dans le service
//$parametres[encrypt]=false; // facultatif
  $parametres[code_appli]="webcal";
// appel service web
 ...
 if ($ldapauth[ConnOk]) {
    ...
    "cal_login = '" . $login . "'";
  	$res = dbi_query ( $sql );
	if ($res) { // on met à jour la base car l'enr existe
		$sql=" UPDATE webcal_user SET cal_passwd='Auth via LDAP', cal_lastname='$ldapauth[Nom]',
		cal_firstname='$ldapauth[Prenom]', cal_email= '$ldapauth[Mail]' WHERE cal_login='$login'";
		$res2=dbi_query ( $sql );
		if (!$res2) {
			$error="Erreur MAJ user dans base locale";
			$ret=false;
			}
		}
	else { // on crée l'enregistrement
		$sql=" INSERT INTO webcal_user cal_login, cal_passwd, cal_lastname, cal_firstname, cal_isadmin, cal_email 
		VALUES (
		'$login',
		'Auth via LDAP',
		'$ldapauth[Nom]',
		...
		$res2=dbi_query ( $sql );
		if (!$res2) {
			$error="Erreur insertion user dans base locale";
			$ret=false;
			}
	...
	} // fin si authenitification LDAP OK

else { // 
	$error= "L'authentification LDAP a échoué: ".$ldapauth[mess_err];
	 $ret = false; // found login/password
	}
/* Méthode d'orgine
  $sql = "SELECT cal_login FROM webcal_user WHERE " .
