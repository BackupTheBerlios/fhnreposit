...
if ( $is_admin ) {
  ...
  if ( $single_user != "Y" ) 
  {
    ...
    if ( $login != "__public__" && (!$user || $user == $login) ) 
    {
      echo " | <a title=\"" . translate("Today") . "\" style=\"font-weight:bold;\" href=\"day.php\">" .  translate("Today") . "</a>\n";
    }

    //if ( $login != "__public__" && $readonly == "N" && ( $require_approvals == "Y" || $public_access == "Y" ) ) 
    if ( $login != "__public__" && (!$user || $user == $login) && $readonly == "N") 
    {
  ...
  if ( empty ( $user ) || $user == $login ) 
  {
    if ( $can_add ) 
    {
      ...
      {
        print "?year=$thisyear";
        if ( ! empty ( $thismonth ) ) {  print "&amp;month=$thismonth"; }
        if ( ! empty ( $thisday ) ) { print "&amp;day=$thisday"; }
      }
