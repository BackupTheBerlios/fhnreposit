...
//   $event_picto - picto associated with this event

function print_entry ( $id, $date, $time, $duration,
  $name, $description, $status,
  $pri, $access, $event_owner,$event_picto="") {
  ...
  if ( $res ) {
    ...
    while ( $row = dbi_fetch_row ( $res ) ) {
      ...
      $item = array (
        ...
        "cal_status" => $row[9],
        "cal_category" => $row[10],
        "cal_picto"=>get_cat_picto($row[10]),
        "cal_login" => $row[11],
    ...
    for ( $i = 0; $i < count ( $result ); $i++ ) {
      ...
      if ( ! empty ( $result[$i]['cal_id'] ) ) {
        $res = dbi_query ( "SELECT cal_date FROM webcal_entry_repeats_not " .
            "WHERE cal_id = " . $result[$i]['cal_id'] ,true,true," function <b>query_events <u>exceptions</u></b>");
        while ( $row = dbi_fetch_row ( $res ) ) {
      ...
      if (! empty($cal_idIN)) {
        $cal_idIN=substr($cal_idIN,0,strlen($cal_idIN)-1);// enlève dernière virgule
        $cal_idIN="(".$cal_idIN.")"; 
        $res = dbi_query ( "SELECT cal_date FROM webcal_entry_repeats_not " .
            "WHERE cal_id IN " . $cal_idIN ,true,true," function <b>query_events <u>exceptions</u></b>");
        while ( $row = dbi_fetch_row ( $res ) ) {
          $result[$i]['cal_exceptions'][] = $row[0];
        }
      }
...
function print_date_entries ( $date, $user, $ssi, $defusers='' ) {
  ...
  // infobule
  $tmp = explode(',', $defusers);
  if(sizeof($tmp) > 1) {
    $infobulle = translate("New Entry All Users");
  } else {
    $infobulle = translate("New Entry");
  }
  ...
  if ( ! $ssi && $can_add ) {
    print "<a title=\"" .
      $infobulle . "\" href=\"edit_entry.php?";
    if ( strcmp ( $user, $GLOBALS["login"] ) )
      print "user=$user&amp;";
    print "date=$date&defusers=$defusers\"><img src=\"new.gif\" alt=\"" .
      $infobulle . "\" class=\"new\" /></a>";
    $cnt++;
  }
  if ( ! $ssi ) {
  ...
  for ( $i = 0; $i < count ( $ev ); $i++ ) {
    ...
    while ( $cur_rep < count ( $rep ) &&
      ...
      if ( $get_unapproved || $rep[$cur_rep]['cal_status'] == 'A' ) {
        ...
        print_entry ( $viewid,
          ...
          $rep[$cur_rep]['cal_access'], $rep[$cur_rep]['cal_login'], $rep[$cur_rep]['cal_picto'] );
        $cnt++;
      }
      $cur_rep++;
    }
    if ( $get_unapproved || $ev[$i]['cal_status'] == 'A' ) {
      ...
      print_entry ( $viewid,
        ...
        $ev[$i]['cal_access'], $ev[$i]['cal_login'], $ev[$i]['cal_picto'] );
      $cnt++;
    }
  }
...
function html_for_add_icon ( $date=0, $hour="", $minute="", $defusers=""  ) {
  global $TZ_OFFSET;
  global $login;
  ...
  return "<a title=\"" . 
	$infobulle . "\" href=\"edit_entry.php?user=" . $login .
    "&date=$date" . ( $hour > 0 ? "&amp;hour=$hour" : "" ) .
    ( $minute > 0 ? "&amp;minute=$minute" : "" ) ."&defusers=$defusers\"><img src=\"new.gif\" class=\"new\" alt=\"" . 
	$infobulle  . "\" /></a>\n";
 
}

function get_cat_picto($cat_id) {
        ...
        if(!$_SESSION['picto']) 
        {
                ...
                while ( $row = dbi_fetch_row ( $res ) ) 
                {
                        $array_picto[$row[0]] = $row[3];
                }
                dbi_free_result ( $res );
                $_SESSION['picto']=$array_picto;
        }       
        return ($_SESSION['picto'][$cat_id]);
...
function html_for_event_week_at_a_glance ( $id, $date, $time,
  $name, $description, $status, $pri, $access, $duration, $event_owner,$event_picto="" ) {
  ...
  // affiche le picto
  $hour_arr[$ind] = $event_picto.' '.$hour_arr[$ind];
    ...
    while ( $cur_rep < count ( $rep ) &&
      ...
      if ( $get_unapproved || $rep[$cur_rep]['cal_status'] == 'A' ) {
        ...
        html_for_event_day_at_a_glance ( $viewid,
          ...
          $rep[$cur_rep]['cal_login'] ,$rep[$cur_rep]['cal_picto']);
      }
      $cur_rep++;
    }
    if ( $get_unapproved || $ev[$i]['cal_status'] == 'A' ) {
...
function load_user_categories ($ex_global = '') {
  global $login;
  global $categories, $category_owners, $category_picto;
  ...
  if ( $categories_enabled == "Y" ) {
    $sql = "SELECT cat_id, cat_name, cat_owner, cat_picto FROM webcal_categories WHERE ";
...
function print_category_menu ( $form, $date = '', $cat_id = '' ) {
  ...
  echo "<b>".translate ('Category')."</b>: <select name=\"cat_id\">\n";
  echo "<option value=\"\"";
  ...
  echo "</select>\n";
  echo "<input type=\"submit\" value=\"".translate('Go to') ."\">\n";
  echo "</form>\n";
