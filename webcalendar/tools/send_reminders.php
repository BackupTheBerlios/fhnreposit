  ...
  for ( $j = 0; $j < count ( $mailusers ); $j++ ) {
    ...
    if ( ! empty ( $server_url ) ) {
      if ( substr ( $server_url, -1, 1 ) == "/" ) {
        $body .= '<a href="'.$server_url.'view_entry.php?id='.$id.'">'.$server_url.'view_entry.php?id='.$id.'</a><br><br>'."\n\n";
      } else {
        $body .= '<a href="'.$server_url.'/view_entry.php?id='.$id.'">'.$server_url.'/view_entry.php?id='.$id.'<a><br><br>'."\n\n"; 
      }
    }
    */
    ...
    if ( $row[2] >= 0 )
      $body .= "<b>".translate ("Time") . ":</b> " . display_time ( $row[2] ) . "<br>\n";
    if ( $row[5] > 0 )
      $body .= "<b>".translate ("Duration") . ":</b> " . $row[5] .
        " " . translate("minutes") . "<br>\n";
    if ( ! $disable_priority_field )
      $body .= "<b>".translate("Priority") . ":</b> " . $pri[$row[6]] . "<br>\n";
    if ( ! $disable_access_field )
      $body .= "<b>".translate("Access") . ":</b> " .
        ( $row[8] == "P" ? translate("Public") : translate("Confidential") ) .
        "<br>\n";
    if ( ! strlen ( $single_user_login ) )
      //$body .= "<b>".translate("Created by") . ":</b> " . $row[0] . "<br>\n";
...
#    }
    ...
    $body .= '<br><a href="http://intranet/webcalendar/">http://intranet/webcalendar/</a>';
